...
void main() {
  test('Should return a list of Books', () async {
      final repository = BookRepositoryMock();

      final List<BookModel> expectedBookList = jsonReturn
        .map((bookData) => BookModel.fromJson(bookData))
        .toList();

      when(() => repository.listBooks()).thenAnswer((_) async => expectedBookList);

      final List<BookModel> list = await repository.listBooks();

      expect(list.first.author, equals("Oswald, Felix L."));
      expect(list.isNotEmpty, equals(true));
  });

  test('Should fetch a new book', () async {
